...
func (u *UserRepositoryImpl) FindAll(ctx context.Context, page, pageSize int) ([]*domain.User, int64, error) {
	...
	if err := u.DB.WithContext(ctx).Limit(page).Offset(offset).Find(&user).Error; err != nil {
		return nil, 0, err
	}
...
func (u *UserRepositoryImpl) Delete(ctx context.Context, userID uint) error {
	if err := u.DB.WithContext(ctx).Delete(&userID).Error; err != nil {
